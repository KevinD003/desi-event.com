...
| 3   | Organizer verification, public routes, venues, venue maps, event lifecycle | **PARTIAL** — organiser verification and the whole venue / venue-map slice are built, tested and on screen; the event lifecycle state machine, moderation, publication gates and cancellation landed in `8661bbf`; event authoring and the organiser event UI are not built |
