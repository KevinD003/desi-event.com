...
export function RequestActions({ organizationId, request, canConfirm, canCancel }) {
  ...
  /**
   * Open one of the forms, remembering where focus came from.
   *
  ...
  /**
   * Close the form and put focus back where it was.
   *
   * @returns {void}
   */
  function dismiss() {
      ...
      {pending === null ? (
        <div className="mt-3 flex flex-wrap gap-3">
          {canConfirm ? (
            <button
              type="button"
              onClick={(event) => open('confirm', event)}
          ...
          {canCancel ? (
            <button
              type="button"
              onClick={(event) => open('cancel', event)}
