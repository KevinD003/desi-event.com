...
when('worker against live Redis', () => {
  ...
  it('carries a payload through Redis and runs the real processor', async () => {
  ...
  it('applies the queue retry policy to jobs it enqueues', async () => {
    const job = await enqueueSendEmail(queues, {
      to: 'buyer@example.com',
  ...
  it('sweeps real holds through the holds queue', async () => {
    ...
    try {
      ...
      const result = await job.waitUntilFinished(holdEvents, 15_000)
  ...
  it('fails a malformed payload without burning its retries', async () => {
    ...
    try {
      await expect(job.waitUntilFinished(searchEvents, 15_000)).rejects.toThrow(/Invalid/)
